...
This is a WIP fork of rfvgyhn's version for 11.7.8
...
## Quick Start
    ...
    docker run -d --name tmod -p 7777:7777 -v /etc/localtime:/etc/localtime:ro johanniklaslp/tmodloader

# Adding worlds

    docker run -d --name tmod -p 7777:7777 -v /etc/localtime:/etc/localtime:ro -v $(pwd)/data:/terraria johanniklaslp/tmodloader
...
You can mount a config file. This allows you to specify server and world settings. If you don't specify one, a [default] will be used. See [wiki] for file format details.
    ...
    docker run -d --name tmod -p 7777:7777 -v /etc/localtime:/etc/localtime:ro -v $(pwd)/data:/terraria -v $(pwd)/config.txt:/terraria-server/config.txt johanniklaslp/tmodloader
...
If you want to use the server's mod browser to install and enable mods, run an interactive container with the `setup` parameter appended to the end.
    ...
    docker run -it --rm -v $(pwd)/data:/terraria johanniklaslp/tmodloader setup
...
# Sample Docker Compose
    ...
    services:
        tmod:
            image: 'johanniklaslp/tmodloader:latest'
            container_name: 'tmod'
            ports:
                - '7777:7777'
            volumes:
